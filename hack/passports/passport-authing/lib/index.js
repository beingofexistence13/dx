...
function Strategy(options, verify) {
  ['domain',
    ...
    if(!options[k]){
      throw new Error('You must provide the ' + k + ' configuration value to use passport-authing.');
    }
  });

  options.customHeaders = Object.assign(
    { 'Authing-Client': clientInfoHeader },
    typeof options.customHeaders === 'object' ? options.customHeaders : {}
  );

  var defaultOptions = {
    expectedIssuer:   'https://' + options.domain + '/',
    authorizationURL: 'https://' + options.domain + '/',
    tokenURL:         'https://' + options.domain + '/oidc/token',
    userInfoURL:      'https://' + options.domain + '/oidc/me',
...
Strategy.prototype.authenticate = function (req, options) {
  ...
  if (this.options.state) {
    if (!req.session) {
      throw new Error('Authing Strategy requires you set state to false when no session is present')
    }
    ...
    throw new Error('Scope "openid" is not allowed without AuthingStrategy state true')
  }
...
/**
 * @deprecated An Access Token is already included as part of the verify callback passed to AuthingStrategy.
 */
 ...
 *   - `id`               this is the user_id of the authing profile
 *   - `username`         this is the nickname of the authing profile
 *   - `displayName`
 *
