var FanfouStrategy = require('../lib/strategy')
  , fs = require('fs');
...
describe('Strategy#userProfile', function() {
  ...
  describe('fetched from default endpoint', function() {
    var strategy = new FanfouStrategy({
      consumerKey: 'ABC123',
      consumerSecret: 'secret'
    }, function verify(){});

    strategy._oauth.get = function(url, token, tokenSecret, callback) {
      if (url != 'http://api.fanfou.com/account/verify_credentials.json') { return callback(new Error('incorrect url argument')); }
    ...
    it('should parse profile', function() {
      expect(profile.provider).to.equal('fanfou');
  ...
  describe('fetched from default endpoint, with email included', function() {
    ...
    strategy._oauth.get = function(url, token, tokenSecret, callback) {
      if (url != 'http://api.fanfou.com/account/verify_credentials.json?include_email=true') { return callback(new Error('incorrect url argument')); }
      if (token != 'token') { return callback(new Error('incorrect token argument')); }
      if (tokenSecret != 'token-secret') { return callback(new Error('incorrect tokenSecret argument')); }
  ...
  describe('fetched from default endpoint, with status excluded', function() {
    ...
    strategy._oauth.get = function(url, token, tokenSecret, callback) {
      if (url != 'http://api.fanfou.com/account/verify_credentials.json?skip_status=true') { return callback(new Error('incorrect url argument')); }
      if (token != 'token') { return callback(new Error('incorrect token argument')); }
      if (tokenSecret != 'token-secret') { return callback(new Error('incorrect tokenSecret argument')); }
  ...
  describe('fetched from default endpoint, with entities excluded', function() {
    ...
    strategy._oauth.get = function(url, token, tokenSecret, callback) {
      if (url != 'http://api.fanfou.com/account/verify_credentials.json?include_entities=false') { return callback(new Error('incorrect url argument')); }
      if (token != 'token') { return callback(new Error('incorrect token argument')); }
      if (tokenSecret != 'token-secret') { return callback(new Error('incorrect tokenSecret argument')); }
  ...
  describe('fetched from legacy users/show endpoint', function() {
    ...
    it('should parse profile', function() {
      ...
      expect(profile.username).to.equal('fanfouapi');
      expect(profile.displayName).to.equal('Fanfou API');
      expect(profile.photos[0].value).to.equal('http://a0.twimg.com/profile_images/1438634086/avatar_normal.png');
    });
