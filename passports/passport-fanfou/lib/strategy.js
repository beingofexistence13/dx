 ...
 *
 * The Fanfou authentication strategy authenticates requests by delegating to
 * Fanfou using the OAuth protocol.
 *
 ...
 *
 * Options:
 *   - `consumerKey`     identifies client to Fanfou
 *   - `consumerSecret`  secret used to establish ownership of the consumer key
 *   - `callbackURL`     URL to which Fanfou will redirect the user after obtaining authorization
 *
 * Examples:
 *
 *     passport.use(new FanfouStrategy({
 *         consumerKey: '123-456-789',
 *         consumerSecret: 'shhh-its-a-secret'
 *         callbackURL: 'https://www.example.net/auth/fanfou/callback'
 *       },
...
function Strategy(options, verify) {
  options = options || {};
  options.requestTokenURL = options.requestTokenURL || 'http://fanfou.com/oauth/request_token';
  options.accessTokenURL = options.accessTokenURL || 'http://fanfou.com/oauth/access_token';
  options.userAuthorizationURL = options.userAuthorizationURL || 'http://fanfou.com/oauth/authorize';
  options.signatureMethod = options.signatureMethod || 'HMAC-SHA1';
  options.sessionKey = options.sessionKey || 'oauth:fanfou';

  OAuthStrategy.call(this, options, verify);
  this.name = 'fanfou';
  this._userProfileURL = options.userProfileURL || 'http://api.fanfou.com/account/verify_credentials.json';
...
/**
 * Authenticate request by delegating to Fanfou using OAuth.
 *
...
Strategy.prototype.authenticate = function(req, options) {
  // When a user denies authorization on Fanfou, they are presented with a link
  // to return to the application in the following format (where xxx is the
  // value of the request token):
  //
  //     http://www.example.com/auth/fanfou/callback?denied=xxx
  //
...
/**
 * Retrieve user profile from Fanfou.
 *
 ...
 *
 * Note that because Fanfou supplies basic profile information in query
 * parameters when redirecting back to the application, loading of Fanfou
...
Strategy.prototype.userProfile = function(token, tokenSecret, params, done) {
  if (!this._skipExtendedUserProfile) {
    ...
    this._oauth.get(uri.format(url), token, tokenSecret, function (err, body, res) {
      ...
      profile.provider = 'fanfou';
      profile._raw = body;
      profile._json = json;
      // NOTE: The "X-Access-Level" header is described here:
      //       https://dev.fanfou.com/oauth/overview/application-permission-model
      //       https://dev.fanfou.com/oauth/overview/application-permission-model-faq
      profile._accessLevel = res.headers['x-access-level'];

      done(null, profile);
    });
  } else {
    var profile = { provider: 'fanfou' };
...
/**
 * Return extra Fanfou-specific parameters to be included in the user
...
/**
 * Parse error response from Fanfou OAuth endpoint.
 *
 * @param {string} body
 * @param {number} status
 * @return {Error}
