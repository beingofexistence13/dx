var AuthingStrategy = require('../lib');
var assert = require('assert');
var should = require('should');
var pkg = require('../package.json');

describe('authing strategy', function () {
  beforeEach(function () {
    this.strategy = new AuthingStrategy({
       domain:       'test.authing.cn',
       clientID:     'testid',
  ...
  describe('options', function() {
    describe('defaults', function() {
      it('expectedIssuer should have the domain', function () {
        ...
        .expectedIssuer.should.eql('https://test.authing.cn/');
      });

      it('authorizationURL should have the domain', function () {
        this.strategy.options
          .authorizationURL.should.eql('https://test.authing.cn/');
      });

      it('tokenURL should have the domain', function () {
        this.strategy.options
          .tokenURL.should.eql('https://test.authing.cn/oidc/token');
      });

      it('userInfoURL should have the domain', function () {
        this.strategy.options
          .userInfoURL.should.eql('https://test.authing.cn/oidc/me');
      });

      it('apiURL should have the domain', function () {
        this.strategy.options
          .apiUrl.should.eql('https://test.authing.cn/api');
      });
    });

    it('should not override options with defaults', function() {
      const strategy = new AuthingStrategy({
          domain:       'test.authing.cn',
          clientID:     'testid',
          clientSecret: 'testsecret',
          callbackURL:  '/callback',
  ...
  it('should include a telemetry header by default', function() {
    ...
    should.exist(headers['Authing-Client']);
  });

  it('should include a correct telemetry values', function() {
    var telemetryValue = new Buffer( this.strategy.options.customHeaders['Authing-Client'], 'base64' ).toString('ascii');
    var telemetryJson = JSON.parse(telemetryValue)

    telemetryJson.name.should.eql('passport-authing');
  ...
  it('should copy options object without mutating', function () {
    ...
    var strategy = new AuthingStrategy(
      options,
      function(accessToken, idToken, profile, done) {}
    );
  ...
  describe('authorizationParams', function () {
    ...
    it('should map the login_hint field', function () {
      var extraParams = this.strategy.authorizationParams({login_hint: 'test.user@authing.cn'});
      extraParams.login_hint.should.eql('test.user@authing.cn');
    });
...
describe('authing strategy with a custom header', function () {
  var strategy = new AuthingStrategy(
    {
      domain:       'test.authing.cn',
      clientID:     'testid',
  ...
  it('should keep the telemetry header', function() {
    should.exist(strategy.options.customHeaders['Authing-Client']);
  });
});

describe('authing strategy with state parameter disabled', function () {
  var strategy = new AuthingStrategy({
    domain:       'test.authing.cn',
    clientID:     'testid',
    clientSecret: 'testsecret',
...
describe('authing strategy with state parameter enabled explicitly', function () {
  var strategy = new AuthingStrategy({
    domain:       'test.authing.cn',
    clientID:     'testid',
    clientSecret: 'testsecret',
